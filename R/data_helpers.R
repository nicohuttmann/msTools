...
#'
#' Chooses the first column if nothing is supplied to argument by.
#'
...
alignexp_tibble_rows <- function(x, by = NULL) {
  ...
  x_joined <- tibble::tibble(!!by := x %>%
                       purrr::map(\(x) dplyr::pull(x, by)) %>%
                       unlist() %>%
                       unique())

  for (i in seq_along(x)) {
    x[[i]] <- dplyr::left_join(x_joined, x[[i]], by = by)
  }
